...
function startAnalysis() {
    ...
    // Запускаем анализ с небольшой задержкой, чтобы UI успел обновиться
    setTimeout(() => {
...
function performAnalysis(resultsDiv) {
    ...
    const isSafe = checkBoundedness();
    html += `
    <div class="analysis-item ${isSafe ? 'safe' : 'unsafe'}">
        <h4>Безопасность (ограниченность)</h4>
        <p>${isSafe 
            ? '✅ Все позиции ограничены (не более одной фишки). Сеть безопасна.' 
            : '⚠️ Некоторые позиции могут содержать более одной фишки. Сеть не является безопасной.'}</p>
    </div>`;

    // 2. Проверка живости (Liveness)
    const livenessResult = checkLiveness();
    html += `
    <div class="analysis-item ${livenessResult.level}">
        <h4>Живость</h4>
        <p>${livenessResult.message}</p>
    </div>`;

    // 3. Анализ достижимости (упрощённый)
    const reachableStates = new Set();
    simulateReachableStates(reachableStates, 100); // Ограничиваем число шагов
    html += `
    <div class="analysis-item">
        <h4>Достижимость</h4>
        <p>Обнаружено приблизительно <strong>${reachableStates.size}</strong> уникальных состояний маркировки.</p>
        <p>Это означает, что сеть может перейти в множество различных конфигураций.</p>
    </div>`;
...
function checkBoundedness() {
    // Простая проверка: если есть дуга с весом >1 на выходе из перехода в позицию,
    // и нет контроля, то позиция может стать неограниченной
    for (const transition of network.transitions) {
        for (const arcId of transition.output) {
            const arc = network.arcs[arcId];
            if (arc && arc.weight > 1) {
                return false; // Может быть неограниченной
            }
        }
    }
    // Также проверяем текущее состояние
    for (const posId in network.positions) {
        if (network.positions[posId].tokens > 1) {
            return false;
        }
    }
    return true;
...
function checkLiveness() {
    if (network.transitions.length === 0) {
        return { level: 'dead', message: '❌ Сеть мертва: нет переходов.' };
    }

    let deadTransitions = 0;
    const totalTransitions = network.transitions.length;

    for (const transition of network.transitions) {
        // Проверяем, может ли переход когда-либо сработать
        const canFire = canTransitionEverFire(transition);
        if (!canFire) {
            deadTransitions++;
        }
    }

    if (deadTransitions === 0) {
        return { 
            level: 'live', 
            message: '✅ Сеть L-живая: каждый переход может сработать бесконечно часто.' 
        };
    } else if (deadTransitions < totalTransitions) {
        return { 
            level: 'quasi-live', 
            message: `🟡 Сеть квазиживая: ${totalTransitions - deadTransitions}/${totalTransitions} переходов могут сработать.` 
        };
    } else {
        return { 
            level: 'dead', 
            message: '❌ Сеть мертва: ни один переход не может сработать.' 
        };
    }
}

function canTransitionEverFire(transition) {
    // Упрощённая проверка: достаточно ли фишек на входах?
    // (без учёта ингибиторных дуг и guard-условий)
    return transition.input.every(arcId => {
        ...
        if (arc.isInhibitor) {
            // Для ингибитора достаточно, чтобы позиция *могла* быть пустой
            return true;
        } else {
            // Обычная дуга: нужно хотя бы weight фишек
            return position.tokens >= arc.weight;
        }
    });
}

function simulateReachableStates(statesSet, maxSteps = 100) {
    const visited = new Set();
    const queue = [{ marking: getCurrentMarking(), steps: 0 }];

    while (queue.length > 0 && queue[0].steps < maxSteps) {
        const { marking, steps } = queue.shift();
        ...
        for (const transition of network.transitions) {
            if (canTransitionFireInMarking(transition, marking)) {
                ...
                queue.push({ marking: newMarking, steps: steps + 1 });
            }
        }
    }
}
...
function canTransitionFireInMarking(transition, marking) {
    return transition.input.every(arcId => {
        ...
        const tokens = marking[posId] || 0;
        return (arc.isInhibitor && tokens === 0) ||
